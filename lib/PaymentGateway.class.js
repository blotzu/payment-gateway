...
module.exports = class PaymentGateway {
    constructor(args) {
        this.amexCurrency = args.amexCurrency || null;
    ...
    pay(paymentDetails, callback) {
        // check for sanity
        if (paymentDetails.isAmex() && paymentDetails.getCurrency() !== this.amexCurrency) {
            return setImmediate(() => callback(`Amex cards only support transactions in ${this.amexCurrency}`));
        }
